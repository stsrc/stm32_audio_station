...
#define FATFS_CS_HIGH GPIOE->BSRR |= (1 << 11)
#define FATFS_CS_LOW GPIOE->BSRR |= (1 << 11) << 0x10
...
static void init_spi (void) {
	...
	GPIOE->OSPEEDR |= GPIO_OSPEEDR_OSPEED11_0 | GPIO_OSPEEDR_OSPEED11_1;
...
)
{
	...
	do {
		d = SPI_4_send(0xFF);
	} while (d != 0xFF);	/* Wait for card goes ready */
	return (d == 0xFF) ? 1 : 0;
}
...
{
	FATFS_CS_HIGH;			/* CS = H */
	SPI_4_send(0xFF);			/* Dummy clock (force DO hi-z for multiple slave SPI) */
...
{
	FATFS_CS_LOW;
	SPI_4_send(0xFF);	/* Dummy clock (force DO enabled) */
...
)
{
	...
	SPI_4_send(0xFF); SPI_4_send(0xFF);			// Discard CRC
...
)
{
	...
	SPI_4_send( token);					/* Send token */
	if (token != 0xFD) {				/* Send data if token is other than StopTran */
		xmit_spi_multi(buff, 512);		/* Data */
		SPI_4_send( 0xFF); SPI_4_send( 0xFF);	/* Dummy CRC */

		resp = SPI_4_send( 0xFF);				/* Receive data resp */
...
)
{
	...
	/* Send command packet */
	SPI_4_send( 0x40 | cmd);				/* Start + command index */
	SPI_4_send( (BYTE)(arg >> 24));		/* Argument[31..24] */
	SPI_4_send( (BYTE)(arg >> 16));		/* Argument[23..16] */
	SPI_4_send( (BYTE)(arg >> 8));		/* Argument[15..8] */
	SPI_4_send( (BYTE)arg);				/* Argument[7..0] */
	...
	if (cmd == CMD12) {
		SPI_4_send( 0xFF);					/* Diacard following one byte when CMD12 */
	}
...
DSTATUS TM_FATFS_SD_disk_initialize (void) {
	...
	if (send_cmd(CMD0, 0) == 1) {				/* Put the card SPI/Idle state */
		if (send_cmd(CMD8, 0x1AA) == 1) {	/* SDv2? */
			for (n = 0; n < 4; n++) {
				ocr[n] = SPI_4_send( 0xFF);	/* Get 32 bit return value of R7 resp */
			}
		...
		if (TM_FATFS_SD_CardType & CT_SD2) {	/* SDC ver 2.00 */
			if (send_cmd(ACMD13, 0) == 0) {	/* Read SD status */
				...
				if (rcvr_datablock(csd, 16)) {				/* Read partial block */
					for (n = 64 - 16; n; n--) SPI_4_send( 0xFF);	/* Purge trailing data */
